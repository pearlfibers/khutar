 ...
 */
do_action( 'khutar_html_start_before' );
...
<head>
	 ...
	 * @since 2.11
	 */
	do_action( 'khutar_head_start_after' );
	?>
	 ...
	 * @since 2.11
	 */
	do_action( 'khutar_head_end_before' );
	?>
</head>

<body  <?php body_class(); ?> <?php khutar_body_attrs(); ?> >
<?php
/**
 ...
 * @since 2.11
 */
do_action( 'khutar_body_start_after' );
?>
<?php wp_body_open(); ?>
<div class="wrapper">
	<?php
	...
	<header class="<?php echo esc_attr( $header_classes ); ?>" role="banner">
		<a class="khutar-skip-link show-on-focus" href="#content" tabindex="0">
			<?php echo __( 'Skip to content', 'khutar' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</a>
		<?php
		...
		if ( apply_filters( 'khutar_filter_toggle_content_parts', true, 'header' ) === true ) {
			do_action( 'khutar_do_header' );
		}
	 ...
	 * @since 1.0.4
	 */
	do_action( 'khutar_before_primary' );
	?>

	<main id="content" class="khutar-main" role="main">
 ...
 * @since 1.0.4
 */
do_action( 'khutar_after_primary_start' );
