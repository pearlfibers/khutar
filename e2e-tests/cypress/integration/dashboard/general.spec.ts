...
describe('General UI', function () {
	...
	beforeEach(function () {
		cy.loginWithRequest();
		cy.visit('/wp-admin/themes.php?page=khutar-welcome');
	...
	it('Test Logger Flag', function () {
		...
		cy.wait('@saveSettings').then((interception) => {
			expect(interception.response.statusCode).to.equal(200);
			expect(interception.response.body.khutar_logger_flag).to.equal('yes');
		});
	...
	it('URL Hash Tab Focus', function () {
		TABS.map((hash) => {
			cy.reload();
			cy.visit(`/wp-admin/themes.php?page=khutar-welcome#${hash}`);
