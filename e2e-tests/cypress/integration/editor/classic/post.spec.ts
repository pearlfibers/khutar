describe('Posts meta box default settings', function () {
	...
	it('Edit meta box settings "' + postSetup.title + '".', function () {
		...
		cy.get('label[for="khutar_meta_container_full-width"]').click();
		cy.get('label[for="khutar_meta_sidebar_left"]').click();
		cy.get('label[for="khutar_meta_disable_title"]').click();
		cy.get('label[for="khutar_meta_disable_header"]').click();
		cy.get('label[for="khutar_meta_disable_featured_image"]').click();
		cy.get('label[for="khutar_meta_disable_footer"]').click();
		cy.get('label[for="khutar_meta_enable_content_width"]').click();
		cy.get('#khutar_meta_content_width-range').invoke('val', 50).trigger('change');
		cy.wait(1000);
