describe('Single page sidebar', function () {
	...
	it.only('Check sidebar layout', function () {
		...
		cy.updatePageOrPostByRequest(pageId, 'pages', {
			meta: {
				khutar_meta_sidebar: 'full-width',
			},
		}).then(() => {
			cy.visit(pageSetup.url);
			cy.get('.nv-sidebar-wrap').should('not.exist');
	...
	it('Check container layout', function () {
		...
		cy.openkhutarSidebar();

		cy.getControl('khutar_meta_container').find('.components-button').contains('Contained').click();
		cy.updatePost();
		cy.visit(pageSetup.url);
		cy.get('.single-page-container').should('have.class', 'container').and('be.visible');
		cy.get('#wp-admin-bar-edit a').click();

		cy.getControl('khutar_meta_container').find('.components-button').contains('Full Width').click();
		cy.updatePost();
		cy.visit(pageSetup.url);
	...
	it('Check container width', function () {
		...
		cy.get('.khutar_meta_content_width').find('input[type=number]').type('{selectall}').type('60');
		cy.updatePost();
		cy.visit(pageSetup.url);
	...
	it('Check title alignment', function () {
		...
		cy.get('.khutar_meta_title_alignment .nv-align-center').click();
		cy.updatePost();
		cy.visit(pageSetup.url);
		...
		cy.get('.khutar_meta_title_alignment .nv-align-right').click();
		cy.updatePost();
		cy.visit(pageSetup.url);
		cy.get('.nv-page-title')
		...
		cy.get('.khutar_meta_title_alignment .nv-align-left').click();
		cy.updatePost();
		cy.visit(pageSetup.url);
		cy.get('.nv-page-title')
	...
	it('Check post elements', function () {
		...
		cy.openkhutarSidebar();
		cy.get('.khutar-meta-sortable').should('not.exist');
