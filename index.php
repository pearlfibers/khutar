 ...
 * @since 1.0.0
 */
$container_class = apply_filters( 'khutar_container_class_filter', 'container', 'blog-archive' );
...
?>
	<div class="<?php echo esc_attr( $container_class ); ?> archive-container">
		<div class="row">
			 ...
			 * @since 1.0.0
			 */
			do_action( 'khutar_do_sidebar', 'blog-archive', 'left' );
			?>
			<div class="nv-index-posts blog col">
				 ...
				 * @since 2.3.10
				 */
				do_action( 'khutar_page_header', 'index' );
				 ...
				 * @since 2.3.10
				 */
				do_action( 'khutar_before_posts_loop' );
				...
				if ( have_posts() ) {
					...
					$pagination_type = get_theme_mod( 'khutar_pagination_type', 'number' );
					...
					if ( $should_add_hook ) {
						 ...
						 * - khutar_loop_attachment_before
						 * - khutar_loop_acme_product_before
						 *
						 * @since 2.11
						 */
						do_action( 'khutar_loop_' . $current_post_type . '_before' );
					}
					...
					while ( have_posts() ) {
						...
						if ( $should_add_hook ) {
							 ...
							 *
							 * Possible action names include:
							 * - khutar_loop_attachment_entry_before
							 * - khutar_loop_acme_product_entry_before
							 *
							 * @since 2.11
							 */
							do_action( 'khutar_loop_' . $current_post_type . '_entry_before' );
						}
						 ...
						 * @since 2.11
						 */
						do_action( 'khutar_loop_entry_before' );
						...
						if ( $pagination_type !== 'infinite' ) {
							if ( $post_index === $hook_after_post && $hook_after_post !== - 1 ) {
								 ...
								 * @since 2.3.10
								 */
								do_action( 'khutar_middle_posts_loop' );
							}
							$post_index ++;
						}
						 ...
						 * @since 2.11
						 */
						do_action( 'khutar_loop_entry_after' );
						...
						if ( $should_add_hook ) {
							 ...
							 *
							 * Possible action names include:
							 * - khutar_loop_attachment_entry_after
							 * - khutar_loop_acme_product_entry_after
							 *
							 * @since 2.11
							 */
							do_action( 'khutar_loop_' . $current_post_type . '_entry_after' );
						}
					}
					echo '</div>';
					if ( ! is_singular() ) {
						 ...
						 */
						do_action( 'khutar_do_pagination', 'blog-archive' );
					}
				} else {
				 ...
				 * @since 2.3.10
				 */
				do_action( 'khutar_after_posts_loop' );
				...
				if ( $should_add_hook ) {
					 ...
					 *
					 * Possible action names include:
					 * - khutar_loop_attachment_after
					 * - khutar_loop_acme_product_after
					 *
					 * @since 2.11
					 */
					do_action( 'khutar_loop_' . $current_post_type . '_after' );
				}
				?>
			</div>
			<?php
			do_action( 'khutar_do_sidebar', 'blog-archive', 'right' );
			?>
		</div>
	</div>
<?php
