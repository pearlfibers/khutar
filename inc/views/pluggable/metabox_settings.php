 ...
 *
 * @package khutar\Views\Pluggable
 */

namespace khutar\Views\Pluggable;

use khutar\Core\Dynamic_Css;
 ...
 */
class Metabox_Settings {
	const CONTENT_WIDTH          = 'khutar_meta_content_width';
	const ENABLE_CONTENT_WIDTH   = 'khutar_meta_enable_content_width';
	const CONTAINER              = 'khutar_meta_container';
	const SIDEBAR                = 'khutar_meta_sidebar';
	const TITLE_ALIGNMENT        = 'khutar_meta_title_alignment';
	const DISABLE_HEADER         = 'khutar_meta_disable_header';
	const DISABLE_TITLE          = 'khutar_meta_disable_title';
	const DISABLE_FEATURED_IMAGE = 'khutar_meta_disable_featured_image';
	const DISABLE_FOOTER         = 'khutar_meta_disable_footer';
	const ELEMENTS_ORDER         = 'khutar_post_elements_order';
	const SHOW_AVATAR            = 'khutar_meta_author_avatar';
	/**
	...
	public function init() {
		add_filter( 'khutar_sidebar_position', array( $this, 'filter_sidebar_position' ) );
		add_filter( 'khutar_container_class_filter', array( $this, 'filter_container_class' ), 100 );
		add_filter( 'body_class', array( $this, 'body_classes' ) );
		add_filter( 'khutar_filter_toggle_content_parts', array( $this, 'filter_components_toggle' ), 100, 2 );
		...
		add_filter( 'khutar_layout_single_post_elements_order', array( $this, 'filter_post_elements' ) );
		add_filter( 'khutar_post_title_alignment', array( $this, 'filter_title_alignment' ) );
		add_filter( 'khutar_display_author_avatar', array( $this, 'filter_author_avatar_display' ) );
	}
	...
	private function get_post_id() {
		...
		$post_id = apply_filters( 'khutar_post_meta_filters_post_id', $post->ID );
	...
	public function editor_content_width() {
		...
		wp_add_inline_style( 'khutar-gutenberg-style', $style );
	...
	public function content_width() {
		...
		if ( $container === 'contained' ) {
			$extra_css = sprintf(
				'
			#content.khutar-main .container .alignfull > [class*="__inner-container"],#content.khutar-main .alignwide > [class*="__inner-container"]{
				max-width: %s;
			}
		...
		@media(min-width: 960px) {
			#content.khutar-main ' . esc_attr( $container_class ) . '.alignfull > [class*="__inner-container"],#content.khutar-main ' . esc_attr( $container_class ) . ' .alignwide > [class*="__inner-container"]{
				max-width: ' . $desktop_value . ';
			}
			#content.khutar-main > ' . esc_attr( $container_class ) . ' > .row > .col{ max-width: ' . absint( $meta_value ) . '%' . esc_attr( $important ) . '; }
			#content.khutar-main > ' . esc_attr( $container_class ) . ' > .row > .nv-sidebar-wrap,
			#content.khutar-main > ' . esc_attr( $container_class ) . ' > .row > .nv-sidebar-wrap.shop-sidebar { max-width: ' . absint( $sidebar_width ) . '%' . esc_attr( $important ) . '; }
		}
		';

		wp_add_inline_style( 'khutar-style', Dynamic_Css::minify_css( $style ) );
	}
