...
class Pagination extends Base_View {
	...
	public function init() {
		...
		add_filter( 'khutar_filter_main_script_localization', array( $this, 'filter_localization' ) );
		add_action( 'khutar_do_pagination', array( $this, 'render_pagination' ) );
		add_action( 'khutar_post_navigation', array( $this, 'render_post_navigation' ) );
	}
	...
	public function render_pagination( $context ) {
		...
		if ( ! $this->has_infinite_scroll() && is_paged() ) {
			 ...
			 * @since 2.3.8
			 */
			do_action( 'khutar_before_pagination' );
		}
	...
	public function render_post_navigation() {
		...
		$prev_link = sprintf(
			...
			esc_html__( 'previous', 'khutar' ),
			'%title'
		);
		$next_link = sprintf(
			'<span class="nav-direction">%1$s</span><span>%2$s</span>',
			esc_html__( 'next', 'khutar' ),
			'%title'
		);
