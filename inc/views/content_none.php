...
class Content_None extends Base_View {
	...
	public function init() {
		add_action( 'khutar_do_content_none', array( $this, 'render_content_none' ) );
	}
	...
	public function render_content_none() {
		...
		if ( is_home() && current_user_can( 'publish_posts' ) ) {
			...
			printf(
				...
				esc_html__( 'Ready to publish your first post? %s.', 'khutar' ),
				sprintf(
					/* translators: %1$s is Link to new post, %2$s is Get started here */
					'<a href="%1$s">%2$s</a>',
					esc_url( admin_url( 'post-new.php' ) ),
					esc_html__( 'Get started here', 'khutar' )
				)
			);
	...
	private function render_search_none() {
		echo '<p>';
		esc_html_e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'khutar' );
		echo '</p>';
