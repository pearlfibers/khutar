...
class Nav_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! self::$mega_menu_enqueued && isset( $item->classes ) && in_array( 'khutar-mega-menu', $item->classes ) ) {
			$this->enqueue_mega_menu_style();
		}

		if ( isset( $item->title ) && $item->title === 'divider' ) {
			$output .= '<li role="presentation" class="khutar-mm-divider">';

			return;
		}

		if ( isset( $item->classes ) && in_array( 'khutar-mm-col', $item->classes, true ) ) {
			$output .= '<li class="khutar-mm-col">';
	...
	public function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		if ( $depth >= 1 ) {
			if ( isset( $item->description ) && ! empty( $item->description ) ) {
				...
				$output .= '<span class="khutar-mm-description">' . esc_html( $item->description ) . '</span>';
			}
		}
		$output .= "</li>{$n}";
	}
	...
	public function tweak_mm_heading( $args, $item, $depth ) {
		...
		if ( in_array( 'khutar-mm-heading', $item->classes, true ) ) {
			add_filter(
				'nav_menu_css_class',
				function ( $classes, $nav_item, $args, $depth ) use ( $item ) {
					...
					return array( 'khutar-mm-heading' );
				},
				10,
				4
			);
	...
	public function enqueue_mega_menu_style() {
		...
		wp_register_style( 'khutar-mega-menu', get_template_directory_uri() . '/assets/css/mega-menu' . ( ( khutar_DEBUG ) ? '' : '.min' ) . '.css', array(), apply_filters( 'khutar_version_filter', khutar_VERSION ) );
		wp_style_add_data( 'khutar-mega-menu', 'rtl', 'replace' );
		wp_style_add_data( 'khutar-mega-menu', 'suffix', '.min' );
		wp_enqueue_style( 'khutar-mega-menu' );
