...
class Post_Layout extends Base_View {
	...
	public function init() {
		add_action( 'khutar_do_single_post', array( $this, 'render_post' ) );
	}
	...
	public function render_post( $context ) {
		...
		if ( apply_filters( 'khutar_filter_toggle_content_parts', true, 'title' ) !== true ) {
		...
		if ( apply_filters( 'khutar_filter_toggle_content_parts', true, 'featured-image' ) !== true || ! has_post_thumbnail() ) {
		...
		$content_order = apply_filters( 'khutar_layout_single_post_elements_order', $content_order );


		/** This filter is documented in header-footer-grid/templates/components/component-logo.php */
		$should_add_skip_lazy = apply_filters( 'khutar_skip_lazy', true );
		$skip_lazy_class      = '';
		...
		foreach ( $content_order as $index => $item ) {
			switch ( $item ) {
				...
				case 'thumbnail':
					...
					break;
				case 'content':
					do_action( 'khutar_before_content', 'single-post' );
					echo '<div class="nv-content-wrap entry-content">';
					the_content();
					echo '</div>';
					do_action( 'khutar_do_pagination', 'single' );
					do_action( 'khutar_after_content', 'single-post' );
					break;
				case 'post-navigation':
					do_action( 'khutar_post_navigation' );
					break;
				case 'tags':
					do_action( 'khutar_do_tags' );
					break;
				case 'title':
				...
				case 'author-biography':
					do_action( 'khutar_layout_single_post_author_biography' );
					break;
				case 'related-posts':
					do_action( 'khutar_do_related_posts' );
					break;
				case 'sharing-icons':
					do_action( 'khutar_do_sharing' );
					break;
				case 'comments':
					comments_template();
					break;
	...
	public static function render_post_meta( $is_list = true ) {
		...
		$meta_order = get_theme_mod( 'khutar_post_meta_ordering', $default_meta_order );
		$meta_order = is_string( $meta_order ) ? json_decode( $meta_order ) : $meta_order;
		$meta_order = apply_filters( 'khutar_post_meta_ordering_filter', $meta_order );
		do_action( 'khutar_post_meta_single', $meta_order, $is_list );
		return true;
	}
	...
	private function render_entry_header( $render_meta = true ) {
		...
		do_action( 'khutar_before_post_title' );
		$alignment = apply_filters( 'khutar_post_title_alignment', '' );
