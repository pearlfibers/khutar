...
class Comments extends Base_View {
	...
	public function init() {
		add_action( 'khutar_do_comment_area', array( $this, 'render_comment_form' ) );
	...
	public function render_comment_form() {
		$display_form_first = apply_filters( 'khutar_show_comment_form_first', false );
		...
		if ( ! comments_open() &&
			...
			?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'khutar' ); ?></p>
			<?php
		}
		if ( ! $display_form_first ) {
			comment_form();
	...
	private function maybe_render_comments_navigation() {
		...
		$aria_label = __( 'Comments Navigation', 'khutar' );
		?>
		<nav class="nv-comment-navigation" role="navigation" aria-label="<?php echo esc_html( $aria_label ); ?>">
			<div class="nav-links">
				<div class="nav-previous">
					<?php previous_comments_link( __( 'Previous', 'khutar' ) ); ?>
				</div>
				<div class="nav-next">
					<?php next_comments_link( __( 'Next', 'khutar' ) ); ?>
				</div>
			</div>
		</nav>
		<?php
		...
		switch ( $comment->comment_type ) {
			...
			case 'trackback':
				...
				<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
					<p>
						<?php
						echo esc_html__( 'Pingback:', 'khutar' ) . '&nbsp;';
						comment_author_link();
						edit_comment_link( '(' . esc_html__( 'Edit', 'khutar' ) . ')', '&nbsp;<span class="edit-link">', '</span>' );
						?>
					</p>
				</li>
				<?php
				...
				<article id="comment-<?php comment_ID(); ?>" class="nv-comment-article">
							...
							<div class="comment-author vcard">
								...
								<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
									<time class="entry-date published"
										...
										echo sprintf( esc_html__( '%1$s at %2$s', 'khutar' ), esc_html( get_comment_date() ), esc_html( get_comment_time() ) );
										?>
									</time>
								</a>
							</div>
						...
						<div class="nv-comment-content comment nv-content-wrap">
							...
							<div class="edit-reply">
								<?php edit_comment_link( '(' . esc_html__( 'Edit', 'khutar' ) . ')' ); ?>
								<?php
								comment_reply_link(
									array_merge(
										$args,
										array(
											'reply_text' => esc_html__( 'Reply', 'khutar' ),
											'add_below'  => 'comment',
											'depth'      => $depth,
							...
							<?php if ( '0' === $comment->comment_approved ) { ?>
								<p class="comment-awaiting-moderation">
									<?php echo esc_html__( 'Comment awaiting moderation.', 'khutar' ); ?>
								</p>
							<?php } ?>
						</div>
					</article>
	...
	private function get_comments_title() {
		...
		return apply_filters( 'khutar_filter_comments_title', $comments_title );
	}
