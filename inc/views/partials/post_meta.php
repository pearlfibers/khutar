...
namespace khutar\Views\Partials;
...
class Post_Meta extends Base_View {
	...
	public function init() {
		add_filter( 'khutar_display_author_avatar', array( $this, 'should_display_author_avatar' ) );
		add_action( 'khutar_post_meta_archive', array( $this, 'render_meta_list' ) );
		add_action( 'khutar_post_meta_single', array( $this, 'render_meta_list' ), 10, 2 );
		add_action( 'khutar_do_tags', array( $this, 'render_tags_list' ) );
	}
	...
	public function should_display_author_avatar( $value ) {
		...
		$show_avatar = get_theme_mod( 'khutar_author_avatar', false );
		...
		foreach ( $order as $meta ) {
			switch ( $meta ) {
				case 'author':
					...
					$markup .= self::khutar_get_author_meta();
					$markup .= '</' . $tag . '>';
					break;
				case 'date':
					...
					$show_updated_time = get_theme_mod( 'khutar_show_last_updated_date', false );
				...
				case 'reading':
					...
					$reading_time = apply_filters( 'khutar_do_read_time', '' );
	 ...
	 * @return string | false
	 */
	public static function khutar_get_author_meta() {
		global $post;
		if ( ! isset( $post ) ) {
			return false;
		}
		...
		$gravatar_args  = apply_filters(
			'khutar_gravatar_args',
			array(
				'size' => 20,
			)
		);
		$display_avatar = apply_filters( 'khutar_display_author_avatar', false );
		...
		$link = sprintf(
			...
			esc_attr( sprintf( __( 'Posts by %s', 'khutar' ), $display_name ) ),
			$display_name
		);
	...
	public static function get_time_tags() {
		 ...
		 * @since 2.11
		 */
		$prefixes = apply_filters( 'khutar_meta_date_prefix', $prefixes );
	...
	public static function get_comments() {
		...
		$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'khutar' ), $comments_number );
