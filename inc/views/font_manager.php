...
class Font_Manager extends Base_View {
	...
	final public static function add_google_font( $font_family, $font_weight = '400' ) {
		if ( empty( $font_family ) ) {
			$body_font = get_theme_mod( 'khutar_body_font_family' );
			if ( empty( $body_font ) ) {
				return;
			}
	...
	private function enqueue_google_font( $font, $weights = [] ) {
		...
		$google_fonts = khutar_get_google_fonts();
		 ...
		 * @since 2.11
		 */
		$should_enqueue_locally = apply_filters( 'khutar_load_remote_fonts_locally', false );
		...
		if ( (bool) $should_enqueue_locally && ! $is_admin_context && is_readable( $vendor_file ) ) {
			...
			wp_enqueue_style( 'khutar-google-font-' . str_replace( ' ', '-', strtolower( $font ) ), $url, array(), khutar_VERSION );
		}
	}
}
