...
class Template_Parts extends Base_View {
	...
	public function init() {
		add_action( 'khutar_blog_post_template_part_content', array( $this, 'render_post' ) );
	...
	private function get_article_inner_content() {
		...
		if ( $layout === 'covers' ) {
			...
			$order         = json_decode( get_theme_mod( 'khutar_post_content_ordering', wp_json_encode( $default_order ) ) );
			$style         = '';
	...
	private function get_post_thumbnail() {
		...
		global $khutar_thumbnail_skip_lazy_added;
		...
		$image_class          = '';
		if ( $should_add_skip_lazy && ! isset( $khutar_thumbnail_skip_lazy_added ) ) {
			$image_class                    = 'skip-lazy';
			$khutar_thumbnail_skip_lazy_added = true;
		}
		...
		return apply_filters( 'khutar_blog_post_thumbnail_markup', $markup );
	}
	...
	private function get_layout() {
		$layout = get_theme_mod( 'khutar_blog_archive_layout', 'grid' );

		if ( $layout !== 'default' ) {
			return $layout;
		}

		if ( get_theme_mod( 'khutar_blog_list_alternative_layout', false ) === true ) {
			$layout = 'alternative';
		}
	...
	private function get_meta() {
		...
		ob_start();
		do_action( 'khutar_post_meta_archive', $meta_order );
		$meta = ob_get_clean();
	...
	private function get_excerpt() {
		ob_start();
		do_action( 'khutar_excerpt_archive', 'index' );
	...
	public function link_excerpt_more( $moretag ) {
		...
		$read_more_args = apply_filters(
			'khutar_read_more_args',
			array(
				'text'    => esc_html__( 'Read More', 'khutar' ) . ' &raquo;',
				'classes' => '',
			)
		);
	...
	private function get_ordered_content_parts( $exclude_thumbnail = false ) {
		...
		$order         = get_theme_mod( 'khutar_post_content_ordering', wp_json_encode( $default_order ) );
			...
			switch ( $content_bit ) {
				...
				case 'excerpt':
					...
					$markup .= wp_link_pages(
						array(
							'before'      => '<div class="post-pages-links"><span>' . apply_filters( 'khutar_page_link_before', esc_html__( 'Pages:', 'khutar' ) ) . '</span>',
							'after'       => '</div>',
