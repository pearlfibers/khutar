...
class Header extends Base_View {
	...
	public function hide_last_menu_item_search_in_sidebar() {
		wp_add_inline_style( 'khutar-style', '.header-menu-sidebar-inner li.menu-item-nav-search { display: none; }' );
	}
	...
	private function get_last_menu_items_markup( $items = '' ) {
		...
		foreach ( $additional_items as $item ) {
			if ( $item === 'search' ) {
				...
				$items .= apply_filters( 'khutar_last_menu_item_' . $item, '' );
			}
		}

		return apply_filters( 'khutar_last_menu_item', $items );
	}
	...
	private function get_last_menu_item_setting() {
		...
		$last_menu_setting_slug = apply_filters( 'khutar_last_menu_setting_slug_' . $current_component, 'khutar_last_menu_item' );
	...
	private function get_nav_menu_search( $responsive = false ) {
		...
		$extra_attrs = apply_filters( 'khutar_search_menu_item_filter', '', self::$primary_nav_instance_no );
		$search     .= '<a href="#" class="nv-nav-search-icon" ' . $extra_attrs . '>' . khutar_search_icon() . '</a>';
		$search     .= '<div class="nv-nav-search">';
		if ( $responsive === true ) {
			$search .= '<div class="container close-container">';
			$search .= '<a class="button button-secondary close-responsive-search">' . __( 'Close', 'khutar' ) . '</a>';
			$search .= '</div>';
		}
	...
	private function get_nav_menu_cart( $responsive = false ) {
		...
		$cart .= khutar_cart_icon();
		$cart .= '<span class="screen-reader-text">' . __( 'Cart', 'khutar' ) . '</span>';
	...
	private function before_cart_popup() {
		...
		do_action( 'khutar_before_cart_popup' );
	...
	private function after_cart_popup() {
		ob_start();
		do_action( 'khutar_after_cart_popup' );
		echo '</div>';
		echo '</div>';
		$markup = ob_get_contents();
		ob_end_clean();
