...
const Handle = SortableHandle(() => (
	<Tooltip text={__('Drag to Reorder', 'khutar')}>
		<button
			aria-label={__('Drag to Reorder', 'khutar')}
			className="handle"
			onClick={(e) => {
				e.preventDefault();
			}}
	...
	return (
		<div
			className={classnames([
				'khutar-sortable-item',
				className,
				{ 'no-toggle': !allowsToggle },
			])}
		>
			{allowsToggle && (
				<Tooltip text={__('Toggle Visibility', 'khutar')}>
					<button
						aria-label={__('Toggle Visibility', 'khutar')}
						className="toggle"
						onClick={(e) => {
							e.preventDefault();
							e.stopPropagation();
