...
const MultiSelect = ({ choices, onChange, currentValue, label }) => {
	...
	return (
		<div className="khutar-white-background-control khutar-multiselect">
			...
			<div className="selected-options">
				{values.length ? (
					...
					<span className="placeholder">
						{__('Add items by clicking the ones below.', 'khutar')}
					</span>
				)}
			</div>
			<div className="available-options">
				{addable.length > 0 ? (
					...
					<span className="placeholder">
						{__('All items are already selected.', 'khutar')}
					</span>
				)}
			</div>
		</div>
