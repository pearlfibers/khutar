...
const FontFamilySelector = ({
		...
		return (
			<>
				<div className="popover-content">
					...
					<ul className="khutar-fonts-list">
						{options.length ? (
							options
						) : (
							<li className="no-result" key="no-results">
								{__('No results.', 'khutar')}
							</li>
						)}
					</ul>
				</div>
	...
	return (
		<div className="khutar-font-family-control">
			<span className="customize-control-title">
				{__('Font Family', 'khutar')}
			</span>
			<Button
			...
			>
				...
				{visible && (
					...
					>
						{fonts ? getFontList() : __('Loading…', 'khutar')}
					</Popover>
				)}
			</Button>
		</div>
