<?php
/**
 * Blog layout section.
 *
 * Author:          Andrei Baicus <andrei@pearlfibers.com>
 * Created on:      20/08/2018
 *
 * @package khutar\Customizer\Options
 */

namespace khutar\Customizer\Options;

use khutar\Customizer\Base_Customizer;
use khutar\Customizer\Types\Control;
use khutar\Customizer\Types\Section;

/**
 * Class Layout_Blog
 *
 * @package khutar\Customizer\Options
 */
class Layout_Blog extends Base_Customizer {
	/**
	 * Function that should be extended to add customizer controls.
	 *
	 * @return void
	 */
	public function add_controls() {
		$this->section_blog();
		$this->add_layout_controls();
		$this->add_content_ordering_controls();
		$this->add_post_meta_controls();
		$this->add_typography_shortcut();

		add_action( 'customize_register', [ $this, 'adapt_old_pro' ], PHP_INT_MAX );
	}

	/**
	 * Adapting old pro versions to make them still usable with the old theme version.
	 */
	public function adapt_old_pro() {
		if ( ! defined( 'khutar_PRO_VERSION' ) ) {
			return;
		}

		if ( version_compare( khutar_PRO_VERSION, '1.2.8', '>' ) ) {
			return;
		}

		$changes = [
			'khutar_posts_order'                   => [ 'priority' => 51 ],
			'khutar_post_content_ordering'         => [ 'active_callback' => '__return_true' ],
			'khutar_blog_ordering_content_heading' => [ 'controls_to_wrap' => 6 ],
			'khutar_blog_post_meta_heading'        => [ 'controls_to_wrap' => 3 ],
			'khutar_read_more_options'             => [
				'accordion' => true,
				'expanded'  => false,
				'class'     => 'read-more-options-accordion',
			],
		];

		foreach ( $changes as $control_slug => $props ) {
			foreach ( $props as $prop => $new_value ) {
				$this->change_customizer_object( 'control', $control_slug, $prop, $new_value );
			}
		}

		$this->wpc->remove_control( 'khutar_metadata_options' );
	}

	/**
	 * Add customize section
	 */
	private function section_blog() {
		$this->add_section(
			new Section(
				'khutar_blog_archive_layout',
				array(
					'priority' => 35,
					'title'    => esc_html__( 'Blog / Archive', 'khutar' ),
					'panel'    => 'khutar_layout',
				)
			)
		);
	}

	/**
	 * Add blog layout controls.
	 */
	private function add_layout_controls() {
		$this->add_control(
			new Control(
				'khutar_blog_layout_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
					'label'            => esc_html__( 'Blog Layout', 'khutar' ),
					'section'          => 'khutar_blog_archive_layout',
					'priority'         => 10,
					'class'            => 'blog-layout-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 5,
				),
				'khutar\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'khutar_blog_archive_layout',
				[
					'default'           => 'grid',
					'sanitize_callback' => [ $this, 'sanitize_blog_layout' ],
				],
				[
					'section'  => 'khutar_blog_archive_layout',
					'priority' => 11,
					'choices'  => [
						'default' => [
							'name'  => __( 'List', 'khutar' ),
							'image' => 'data:image/png;base64,',
						],
						'covers'  => [
							'name'  => __( 'Covers', 'khutar' ),
							'image' => 'data:image/png;base64,',
						],
						'grid'    => [
							'name'  => __( 'Grid', 'khutar' ),
							'image' => 'data:image/jpeg;base64,',
						],
					],
				],
				'\khutar\Customizer\Controls\React\Radio_Image'
			)
		);

		$this->add_control(
			new Control(
				'khutar_grid_layout',
				array(
					'sanitize_callback' => 'khutar_sanitize_range_value',
					'default'           => wp_json_encode(
						array(
							'desktop' => 1,
							'tablet'  => 1,
							'mobile'  => 1,
						)
					),
				),
				array(
					'label'           => esc_html__( 'Columns', 'khutar' ),
					'section'         => 'khutar_blog_archive_layout',
					'units'           => array(
						'items',
					),
					'input_attrs'     => [
						'step'       => 1,
						'min'        => 1,
						'max'        => 4,
						'defaultVal' => [
							'mobile'  => 1,
							'tablet'  => 1,
							'desktop' => 1,
						],
					],
					'priority'        => 11,
					'active_callback' => array( $this, 'is_column_layout' ),
				),
				'khutar\Customizer\Controls\React\Responsive_Range'
			)
		);

		$this->add_control(
			new Control(
				'khutar_blog_covers_text_color',
				array(
					'sanitize_callback' => 'khutar_sanitize_colors',
					'default'           => '#ffffff',
					'transport'         => 'postMessage',
				),
				array(
					'label'                 => esc_html__( 'Text Color', 'khutar' ),
					'section'               => 'khutar_blog_archive_layout',
					'priority'              => 15,
					'default'               => '#ffffff',
					'active_callback'       => function () {
						return get_theme_mod( 'khutar_blog_archive_layout' ) === 'covers';
					},
					'live_refresh_selector' => true,
					'live_refresh_css_prop' => [
						'template' =>
							'.cover-post .inner, .cover-post .inner a:not(.button), .cover-post .inner a:not(.button):hover, .cover-post .inner a:not(.button):focus, .cover-post .inner li {
							color: {{value}};
						}',
					],
				),
				'khutar\Customizer\Controls\React\Color'
			)
		);

		$this->add_control(
			new Control(
				'khutar_blog_list_alternative_layout',
				array(
					'sanitize_callback' => 'khutar_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'type'            => 'khutar_toggle_control',
					'priority'        => 17,
					'section'         => 'khutar_blog_archive_layout',
					'label'           => esc_html__( 'Alternating layout', 'khutar' ),
					'active_callback' => function () {
						return get_theme_mod( 'khutar_blog_archive_layout' ) === 'default';
					},
				)
			)
		);

		$this->add_control(
			new Control(
				'khutar_enable_masonry',
				array(
					'sanitize_callback' => 'khutar_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'type'            => 'khutar_toggle_control',
					'priority'        => 35,
					'section'         => 'khutar_blog_archive_layout',
					'label'           => esc_html__( 'Enable Masonry', 'khutar' ),
					'active_callback' => array( $this, 'should_show_masonry' ),
				)
			)
		);
	}

	/**
	 * Add content ordering and controls.
	 */
	private function add_content_ordering_controls() {
		$this->add_control(
			new Control(
				'khutar_blog_ordering_content_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
					'label'            => esc_html__( 'Ordering and Content', 'khutar' ),
					'section'          => 'khutar_blog_archive_layout',
					'priority'         => 50,
					'class'            => 'blog-layout-ordering-content-accordion',
					'accordion'        => true,
					'expanded'         => false,
					'controls_to_wrap' => 4,
				),
				'khutar\Customizer\Controls\Heading'
			)
		);

		$this->add_control(
			new Control(
				'khutar_pagination_type',
				array(
					'default'           => 'number',
					'sanitize_callback' => array( $this, 'sanitize_pagination_type' ),
				),
				array(
					'label'    => esc_html__( 'Post Pagination', 'khutar' ),
					'section'  => 'khutar_blog_archive_layout',
					'priority' => 53,
					'type'     => 'select',
					'choices'  => array(
						'number'   => esc_html__( 'Number', 'khutar' ),
						'infinite' => esc_html__( 'Infinite Scroll', 'khutar' ),
					),
				)
			)
		);

		$order_default_components = array(
			'thumbnail',
			'title-meta',
			'excerpt',
		);

		$components = array(
			'thumbnail'  => __( 'Thumbnail', 'khutar' ),
			'title-meta' => __( 'Title & Meta', 'khutar' ),
			'excerpt'    => __( 'Excerpt', 'khutar' ),
		);

		$this->add_control(
			new Control(
				'khutar_post_content_ordering',
				array(
					'sanitize_callback' => array( $this, 'sanitize_post_content_ordering' ),
					'default'           => wp_json_encode( $order_default_components ),
				),
				array(
					'label'      => esc_html__( 'Post Content Order', 'khutar' ),
					'section'    => 'khutar_blog_archive_layout',
					'components' => $components,
					'priority'   => 55,
				),
				'khutar\Customizer\Controls\React\Ordering'
			)
		);

		$this->add_control(
			new Control(
				'khutar_post_excerpt_length',
				array(
					'sanitize_callback' => 'khutar_sanitize_range_value',
					'default'           => 25,
				),
				array(
					'label'       => esc_html__( 'Excerpt Length', 'khutar' ),
					'section'     => 'khutar_blog_archive_layout',
					'type'        => 'khutar_range_control',
					'input_attrs' => [
						'min'        => 5,
						'max'        => 300,
						'defaultVal' => 25,
						'step'       => 5,
					],
					'priority'    => 58,
				),
				'khutar\Customizer\Controls\React\Range'
			)
		);

		$this->add_control(
			new Control(
				'khutar_post_thumbnail_box_shadow',
				array(
					'sanitize_callback' => 'absint',
					'default'           => 0,
				),
				array(
					'label'       => esc_html__( 'Thumbnail Shadow', 'khutar' ),
					'section'     => 'khutar_blog_archive_layout',
					'type'        => 'khutar_range_control',
					'step'        => 1,
					'input_attrs' => [
						'min'        => 0,
						'max'        => 5,
						'defaultVal' => 0,
					],
					'priority'    => 59,
				),
				'khutar\Customizer\Controls\React\Range'
			)
		);
	}

	/**
	 * Add controls for post meta.
	 */
	private function add_post_meta_controls() {
		$this->add_control(
			new Control(
				'khutar_blog_post_meta_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
					'label'            => esc_html__( 'Post Meta', 'khutar' ),
					'section'          => 'khutar_blog_archive_layout',
					'priority'         => 70,
					'class'            => 'blog-layout-post-meta-accordion',
					'accordion'        => true,
					'controls_to_wrap' => 3,
					'expanded'         => false,
				),
				'khutar\Customizer\Controls\Heading'
			)
		);

		$order_default_components = array(
			'author',
			'date',
			'comments',
		);

		$components = apply_filters(
			'khutar_meta_filter',
			array(
				'author'   => __( 'Author', 'khutar' ),
				'category' => __( 'Category', 'khutar' ),
				'date'     => __( 'Date', 'khutar' ),
				'comments' => __( 'Comments', 'khutar' ),
			)
		);

		$this->add_control(
			new Control(
				'khutar_post_meta_ordering',
				array(
					'sanitize_callback' => array( $this, 'sanitize_meta_ordering' ),
					'default'           => wp_json_encode( $order_default_components ),
				),
				array(
					'label'           => esc_html__( 'Meta Order', 'khutar' ),
					'section'         => 'khutar_blog_archive_layout',
					'components'      => $components,
					'priority'        => 75,
					'active_callback' => array( $this, 'should_show_meta_order' ),
				),
				'khutar\Customizer\Controls\React\Ordering'
			)
		);

		$this->add_control(
			new Control(
				'khutar_author_avatar',
				array(
					'sanitize_callback' => 'khutar_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'label'    => esc_html__( 'Show Author Avatar', 'khutar' ),
					'section'  => 'khutar_blog_archive_layout',
					'type'     => 'khutar_toggle_control',
					'priority' => 80,
				)
			)
		);

		$this->add_control(
			new Control(
				'khutar_show_last_updated_date',
				array(
					'sanitize_callback' => 'khutar_sanitize_checkbox',
					'default'           => false,
				),
				array(
					'label'    => esc_html__( 'Use last updated date instead of the published one', 'khutar' ),
					'section'  => 'khutar_blog_archive_layout',
					'type'     => 'khutar_toggle_control',
					'priority' => 85,
				)
			)
		);

	}

	/**
	 * Sanitize the container layout value
	 *
	 * @param string $value value from the control.
	 *
	 * @return bool
	 */
	public function sanitize_blog_layout( $value ) {
		$allowed_values = array( 'default', 'covers', 'grid' );
		if ( ! in_array( $value, $allowed_values, true ) ) {
			return 'grid';
		}

		return sanitize_text_field( $value );
	}

	/**
	 * Sanitize the pagination type
	 *
	 * @param string $value value from the control.
	 *
	 * @return bool
	 */
	public function sanitize_pagination_type( $value ) {
		$allowed_values = array( 'number', 'infinite' );
		if ( ! in_array( $value, $allowed_values, true ) ) {
			return 'number';
		}

		return esc_html( $value );
	}

	/**
	 * Sanitize meta order control.
	 */
	public function sanitize_meta_ordering( $value ) {
		$allowed = array(
			'author',
			'category',
			'date',
			'comments',
			'reading',
		);

		if ( empty( $value ) ) {
			return $allowed;
		}

		$decoded = json_decode( $value, true );

		foreach ( $decoded as $val ) {
			if ( ! in_array( $val, $allowed, true ) ) {
				return $allowed;
			}
		}

		return $value;
	}

	/**
	 * Sanitize content order control.
	 */
	public function sanitize_post_content_ordering( $value ) {
		$allowed = array(
			'thumbnail',
			'title-meta',
			'excerpt',
		);

		if ( empty( $value ) ) {
			return $allowed;
		}

		$decoded = json_decode( $value, true );

		foreach ( $decoded as $val ) {
			if ( ! in_array( $val, $allowed, true ) ) {
				return $allowed;
			}
		}

		return $value;
	}

	/**
	 * Callback to show the meta order control.
	 *
	 * @return bool
	 */
	public function should_show_meta_order() {
		$default       = array(
			'thumbnail',
			'title-meta',
			'excerpt',
		);
		$content_order = get_theme_mod( 'khutar_post_content_ordering', wp_json_encode( $default ) );
		$content_order = json_decode( $content_order, true );
		if ( ! in_array( 'title-meta', $content_order, true ) ) {
			return false;
		}

		return true;
	}

	/**
	 * Callback to show grid columns control.
	 *
	 * @return bool
	 */
	public function is_column_layout() {
		$blog_layout = get_theme_mod( 'khutar_blog_archive_layout', 'grid' );

		return in_array( $blog_layout, [ 'grid', 'covers' ], true );
	}

	/**
	 * Callback to show masonry control.
	 *
	 * @return bool
	 */
	public function should_show_masonry() {
		if ( ! $this->is_column_layout() ) {
			return false;
		}

		$columns = json_decode( get_theme_mod( 'khutar_grid_layout', '{"desktop":1,"tablet":1,"mobile":1}' ), true );
		$columns = array_filter(
			array_values( $columns ),
			function ( $value ) {
				return $value > 1;
			}
		);

		if ( empty( $columns ) ) {
			return false;
		}

		return true;
	}

	/**
	 * Add typography shortcut.
	 */
	private function add_typography_shortcut() {
		$this->add_control(
			new Control(
				'khutar_blog_typography_shortcut',
				array(
					'sanitize_callback' => 'khutar_sanitize_text_field',
				),
				array(
					'button_class'     => 'nv-top-bar-menu-shortcut',
					'text_before'      => __( 'Customize Typography for the Archive page', 'khutar' ),
					'text_after'       => '.',
					'button_text'      => __( 'here', 'khutar' ),
					'is_button'        => false,
					'control_to_focus' => 'khutar_archive_typography_post_title_accordion_wrap',
					'shortcut'         => true,
					'section'          => 'khutar_blog_archive_layout',
					'priority'         => 1000,
				),
				'\khutar\Customizer\Controls\Button'
			)
		);
	}
}
