...
class Layout_Single_Product extends Base_Customizer {
	...
	private function section_single_product() {
		$this->add_section(
			new Section(
				'khutar_single_product_layout',
				array(
					'priority' => 65,
					'title'    => esc_html__( 'Single Product', 'khutar' ),
					'panel'    => 'woocommerce',
				)
			)
		);
	...
	private function exclusive_products_controls() {
		$this->add_control(
			new Control(
				'khutar_exclusive_products_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
					'transport'         => $this->selective_refresh,
				),
				array(
					'label'            => esc_html__( 'Exclusive Products', 'khutar' ),
		...
		$this->add_control(
			new Control(
				'khutar_exclusive_products_title',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
		...
		$this->add_control(
			new Control(
				'khutar_exclusive_products_category',
				array(
					'default'           => '-',
					'sanitize_callback' => array( $this, 'sanitize_categories' ),
				),
				array(
					'label'    => esc_html__( 'Category', 'khutar' ),
					'section'  => 'khutar_single_product_layout',
					'priority' => 220,
					'type'     => 'select',
					'choices'  => $product_cat,
	...
	private function get_shop_categories() {
		$categories         = array(
			'-'   => esc_html__( 'None', 'khutar' ),
			'all' => esc_html__( 'All', 'khutar' ),
		);
