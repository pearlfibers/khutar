...
class Amp {
	...
	public function register_hooks() {
		...
		add_filter( 'khutar_caret_wrap_filter', array( $this, 'amp_dropdowns' ), 10, 2 );
		add_filter(
			'khutar_woocommerce_sidebar_filter_btn_data_attrs',
			array(
				$this,
		...
		add_filter(
			'khutar_filter_sidebar_close_button_data_attrs',
			array(
				$this,
		...
		add_filter( 'khutar_sidebar_data_attrs', array( $this, 'add_woo_sidebar_attrs' ), 10, 2 );
		add_filter( 'khutar_search_menu_item_filter', array( $this, 'add_search_menu_item_attrs' ), 10, 2 );
		add_action( 'khutar_after_header_hook', array( $this, 'render_amp_states' ) );
		add_filter( 'khutar_nav_toggle_data_attrs', array( $this, 'add_nav_toggle_attrs' ) );
	}
	...
	public function wrap_content( $item_output, $item, $depth, $args ) {
		...
		$item_output = apply_filters( 'khutar_caret_wrap_filter', $item_output, $item->menu_order );
	...
	public function add_nav_toggle_attrs( $input = '' ) {
		$input  = ' on="tap:khutar_body.toggleClass(class=\'is-menu-sidebar\'),AMP.setState( { nvAmpMenuExpanded: ! nvAmpMenuExpanded } )" ';
		$input .= ' role="button" ';
		$input .= ' tabindex="0" ';
	...
	public function amp_dropdowns( $output, $id ) {
		...
		$state = 'khutarMenuItemExpanded' . $id;
