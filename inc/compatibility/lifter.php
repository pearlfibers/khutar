...
class Lifter {
	...
	private function load_hooks() {
		...
		add_action( 'khutar_llms_content', array( $this, 'content_open' ), 10 );
	...
	public function load_styles() {
		wp_enqueue_style( 'khutar-lifter', khutar_ASSETS_URL . 'css/lifter' . ( ( khutar_DEBUG ) ? '' : '.min' ) . '.css', array(), apply_filters( 'khutar_version_filter', khutar_VERSION ) );
	}
	...
	public function load_catalog_sidebar() {
		$sidebar_position = get_theme_mod( 'khutar_default_sidebar_layout', 'right' );
		if ( $sidebar_position === 'right' ) {
			add_action( 'lifterlms_after_main_content', array( $this, 'render_catalog_sidebar' ), 11 );
		}
		if ( $sidebar_position === 'left' ) {
			add_action( 'khutar_llms_content', array( $this, 'render_catalog_sidebar' ), 1 );
		}
	}
	...
	public function content_wrapper_open() {
		$container_class = apply_filters( 'khutar_container_class_filter', 'container' );
		echo '<div class="' . esc_attr( $container_class ) . ' lms-container">';
		echo '<div class="row">';
		do_action( 'khutar_llms_content' );
	}
	...
	public function content_open() {
		...
		echo '</div>';
		echo '</div>';
		$class = apply_filters( 'khutar_lifter_wrap_classes', 'nv-content-wrap entry-content' );
	...
	public function register_catalog_sidebar() {
		register_sidebar(
			array(
				'name'          => __( 'Catalog Sidebar', 'khutar' ),
				'id'            => 'llms_shop',
