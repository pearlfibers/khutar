...
class Starter_Content {
	...
	public function starter_meta( $value, $post_id, $meta_key ) {
		...
		if ( $meta_key === 'khutar_meta_disable_title' ) {
			return 'on';
		}
		if ( $meta_key === 'khutar_meta_enable_content_width' ) {
			return 'on';
		}
		if ( $meta_key === 'khutar_meta_content_width' ) {
			return '100';
		}
	...
	public function register_listener( $post_ID, $post, $update ) {
		...
		if ( $post->post_type === 'page' ) {
			update_post_meta( $post_ID, 'khutar_meta_disable_title', 'on' );
		...
		$content = [
			...
			'posts'       => [
				...
				self::BLOG_SLUG       => [
					...
					'post_type'  => 'page',
					'post_title' => _x( 'Blog', 'Theme starter content', 'khutar' ),
				],
			],
		];


		return apply_filters( 'khutar_starter_content', $content );
	}
}
