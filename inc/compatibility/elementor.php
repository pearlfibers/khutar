...
class Elementor extends Page_Builder_Base {
	...
	public function init() {
		...
		add_action( 'khutar_dynamic_style_output', array( $this, 'fix_links' ), 99, 2 );
	...
	public function enqueue() {
		...
		wp_add_inline_style( 'khutar-style', $css );
	}

	/**
	 * Filter rest responses to add khutar Palette Colors to pages using Elementor.
	 *
	...
	public function alter_global_colors_front_end( $response, $handler, \WP_REST_Request $request ) {
		...
		$response = new \WP_REST_Response(
			[
				...
				'value' => khutar_sanitize_colors( $colors[ $rest_to_slugs[ $rest_id ] ] ),
			]
		);
		return $response;
	}
	...
	/**
	 * Filter rest responses to add khutar Palette Colors to Elementor.
	 *
	 * @param \WP_REST_Response $response request response.
	 * @param array             $handler request handler.
	 * @param \WP_REST_Request  $request rest request.
	...
	public function alter_global_colors_in_picker( $response, $handler, \WP_REST_Request $request ) {
		...
		foreach ( $colors as $slug => $color_value ) {
			...
			$data['colors'][ $no_hyphens ] = [
				...
				'value' => khutar_sanitize_colors( $color_value ),
			];
		}
	...
	public function add_theme_builder_hooks() {
		...
		add_action( 'khutar_do_top_bar', array( $this, 'do_header' ), 0 );
		add_action( 'khutar_do_header', array( $this, 'do_header' ), 0 );
		add_action( 'khutar_do_footer', array( $this, 'do_footer' ), 0 );
		add_action( 'khutar_do_404', array( $this, 'do_404' ), 0 );
		add_action( 'khutar_do_single_post', array( $this, 'do_single_post' ), 0 );
		add_action( 'khutar_do_single_page', array( $this, 'do_single_page' ), 0 );
		add_action( 'khutar_page_header', array( $this, 'remove_header_on_page' ), 0 );
	}
	...
	public function do_404() {
		...
		if ( $did_location ) {
			remove_all_actions( 'khutar_do_404' );
		}
	}
	...
	public function do_single_post() {
		$did_location = $this->elementor_location_manager->do_location( 'single' );
		if ( $did_location ) {
			remove_all_actions( 'khutar_do_single_post' );
		}
	}
	...
	public function do_single_page() {
		$did_location = $this->elementor_location_manager->do_location( 'single' );
		if ( $did_location ) {
			remove_all_actions( 'khutar_do_single_page' );
		}
	}
	...
	public function remove_header_on_page() {
		...
		if ( elementor_theme_do_location( 'single' ) ) {
			remove_all_actions( 'khutar_page_header' );
		}
	}
	...
	/**
	 * Fix the underline of links added by khutar.
	 *
	...
	private function get_current_palette_colors() {
		$customizer = get_theme_mod( 'khutar_global_colors', khutar_get_global_colors_default( true ) );
	...
	private function get_global_color_prefix() {
		return ( apply_filters( 'ti_wl_theme_is_localized', false ) ? __( 'Theme', 'khutar' ) : 'khutar' ) . ' - ';
	}
}
