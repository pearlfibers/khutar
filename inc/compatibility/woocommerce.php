...
use khutar\Views\Layouts\Layout_Sidebar;
...
class Woocommerce {
	...
	private $secondary_buttons_selectors = array(
		'default' => '
			...
			.woocommerce-checkout #khutar-checkout-coupon .woocommerce-form-coupon .form-row-last button,
			...
			.woocommerce-checkout #khutar-checkout-coupon .woocommerce-form-coupon .form-row-last button.button',
		'hover'   => '
			,.woocommerce-cart table.cart td.actions .coupon > .input-text + .button:hover,
			.woocommerce-checkout #khutar-checkout-coupon .woocommerce-form-coupon .form-row-last button:hover,
			...
			.woocommerce-checkout #khutar-checkout-coupon .woocommerce-form-coupon .form-row-last button.button:hover',
	);
	/**
	 * Sidebar manager.
	 *
	 * @var \khutar\Views\Layouts\Layout_Sidebar
	 */
	...
	public function init() {
		...
		// Wrap content.
		add_action( 'khutar_after_primary_start', array( $this, 'wrap_pages_start' ) );
		add_action( 'khutar_before_primary_end', array( $this, 'wrap_pages_end' ) );
		 ...
		 */
		add_filter( 'khutar_sidebar_position', array( $this, 'product_page_sidebar_default_position' ), 9 );
		...
		add_filter( 'khutar_post_meta_filters_post_id', array( $this, 'adapt_meta_for_shop_page' ) );
		...
		add_action( 'khutar_bc_count', 'woocommerce_breadcrumb' );
		add_action( 'khutar_bc_count', 'woocommerce_result_count' );
	...
	public function set_update_woo_width_flag() {
		...
		update_option( 'khutar_update_woo_width', 'yes' );
	...
	public function update_woo_width() {
		if ( get_option( 'khutar_update_woo_width' ) !== 'yes' ) {
			return false;
		}
		...
		foreach ( $pages as $page_id ) {
			...
			update_post_meta( $page_id, 'khutar_meta_sidebar', 'full-width' );
			update_post_meta( $page_id, 'khutar_meta_enable_content_width', 'on' );
			update_post_meta( $page_id, 'khutar_meta_content_width', 100 );
		}
		update_option( 'khutar_update_woo_width', false );
	...
	public function product_page_sidebar_default_position( $input ) {
		...
		if ( is_product() ) {
			return get_theme_mod( 'khutar_single_product_sidebar_layout', 'full-width' );
		}
	...
	private function edit_woocommerce_header() {
		...
		add_action( 'khutar_before_shop_loop_content', array( $this, 'add_header_bits' ), 0 );
	...
	public function wrap_main_content_start() {
		$before_shop_classes = apply_filters( 'khutar_before_shop_classes', 'nv-index-posts nv-shop col' );
		echo '<div class="' . esc_attr( $before_shop_classes ) . '">';
		do_action( 'khutar_before_shop_loop_content' );
	}
	...
	public function wrap_pages_start() {
		...
		echo '<div class="' . esc_attr( apply_filters( 'khutar_container_class_filter', 'container' ) ) . ' shop-container">';
		echo '<div class="row">';
	}
	...
	public function sidebar_toggle() {
		...
		$button_text  = apply_filters( 'khutar_filter_woo_sidebar_open_button_text', __( 'Filter', 'khutar' ) . '»' );
		$button_attrs = apply_filters( 'khutar_woocommerce_sidebar_filter_btn_data_attrs', '' );
	...
	private function add_inline_selectors() {
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_BTN_PRIMARY_NORMAL,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_BTN_PRIMARY_HOVER,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_BTN_PRIMARY_PADDING,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_BTN_SECONDARY_NORMAL,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_BTN_SECONDARY_HOVER,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_BTN_SECONDARY_PADDING,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_FORM_INPUTS,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_FORM_INPUTS_WITH_SPACING,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_FORM_INPUTS_LABELS,
			array(
				$this,
		...
		add_filter(
			'khutar_selectors_' . Config::CSS_SELECTOR_FORM_SEARCH_INPUTS,
			array(
				$this,
		...
		add_filter( 'khutar_body_font_family_selectors', array( $this, 'add_font_families' ) );
		add_filter( 'khutar_headings_typeface_selectors', array( $this, 'add_typeface_selectors' ) );
	}
	...
	private function should_render_sidebar_toggle() {
		...
		if ( is_shop() ) {
			$theme_mod = apply_filters( 'khutar_sidebar_position', get_theme_mod( 'khutar_shop_archive_sidebar_layout', 'right' ) );
			if ( $theme_mod !== 'right' && $theme_mod !== 'left' ) {
				return false;
			}
		}
		if ( is_product() ) {
			$theme_mod = apply_filters( 'khutar_sidebar_position', get_theme_mod( 'khutar_single_product_sidebar_layout', 'right' ) );
			if ( $theme_mod !== 'right' && $theme_mod !== 'left' ) {
				return false;
			}
		}
	...
	public function move_coupon() {
		wc_enqueue_js( '$( $( ".woocommerce-checkout div.woocommerce-info, .checkout_coupon, .woocommerce-form-login" ).detach() ).appendTo( "#khutar-checkout-coupon" );' );
	}

	/**
	 * Checkout page
	 * Add the id khutar-checkout-coupon to be able to Move the coupon field and message info after the order table.
	 */
	public function clear_coupon() {
		echo '<div id="khutar-checkout-coupon"></div><div style="clear:both"></div>';
	}
	...
	public function setup_form_buttons() {
		$form_buttons_type = get_theme_mod( 'khutar_form_button_type', 'primary' );
