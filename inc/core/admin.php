...
class Admin {
	 ...
	 * @var string
	 */
	private $dismiss_notice_key = 'khutar_notice_dismissed';
	/**
	 * Current theme name
	 *
	...
	public function __construct() {
		$this->set_props();
		if ( class_exists( '\khutar_Dash\Main' ) ) {
			new \khutar_Dash\Main();
		}
		add_action(
			'admin_init',
			function () {
				if ( get_option( 'pearlfibers_ob_plugins_installed' ) !== 'yes' ) {
					return;
				}
				update_option( 'pearlfibers_blocks_settings_redirect', false );
				delete_transient( 'wpforms_activation_redirect' );
				update_option( 'pearlfibers_ob_plugins_installed', 'no' );
			},
			0
		);
		add_action( 'enqueue_block_editor_assets', [ $this, 'enqueue_gutenberg_scripts' ] );
		add_filter( 'pearlfibers_sdk_hide_dashboard_widget', '__return_true' );

		if ( get_option( $this->dismiss_notice_key ) !== 'yes' ) {
			add_action( 'admin_notices', [ $this, 'admin_notice' ], 0 );
			add_action( 'wp_ajax_khutar_dismiss_welcome_notice', [ $this, 'remove_notice' ] );
		}
	...
	public function admin_notice() {
		if ( apply_filters( 'khutar_disable_starter_sites_admin_notice', false ) === true ) {
			return;
		}
	...
	public function welcome_notice_content() {
		...
		$notice_header = sprintf(
			...
			esc_html__( 'Congratulations!', 'khutar' ),
			sprintf(
				/* translators: %s - theme name */
				esc_html__( '%s is now installed and ready to use. We\'ve assembled some links to get you started.', 'khutar' ),
				$name
			)
		);
		...
		/* translators: 1 - onboarding url, 2 - button text */
			...
			esc_url( $ob_btn_link ),
			sprintf( apply_filters( 'ti_onboarding_khutar_start_site_cta', esc_html__( 'Try one of our ready to use Starter Sites', 'khutar' ) ) )
		);
		...
		/* translators: 1 - button text */
			...
			__( 'Return to your dashboard', 'khutar' )
		);
		...
		/* translators: 1 - options page url, 2 - button text */
			...
			esc_html__( 'or go to the theme settings', 'khutar' )
		);
		...
		$notice_sites_list = sprintf(
			...
			__( 'Sites Library', 'khutar' ),
			// translators: %s - Theme name
				sprintf( esc_html__( '%s now comes with a sites library with various designs to pick from. Visit our collection of demos that are constantly being added.', 'khutar' ), $name ),
			$ob_btn,
			$options_page_btn
		);
		$notice_documentation = sprintf(
			...
			__( 'Documentation', 'khutar' ),
			// translators: %s - Theme name
				sprintf( esc_html__( 'Need more details? Please check our full documentation for detailed information on how to use %s.', 'khutar' ), $name ),
			'https://docs.pearlfibers.com/article/946-khutar-doc',
	...
	public function enqueue_gutenberg_scripts() {
		...
		wp_enqueue_script(
			'khutar-gutenberg-script',
			khutar_ASSETS_URL . 'js/build/all/gutenberg.js',
			array( 'wp-blocks', 'wp-dom' ),
			khutar_VERSION,
			true
		);
		wp_enqueue_style( 'khutar-gutenberg-style', khutar_ASSETS_URL . 'css/gutenberg-editor-style' . ( ( khutar_DEBUG ) ? '' : '.min' ) . '.css', array(), khutar_VERSION );
	}
					...
					$(actionButton).on('click', function (e) {
						...
						$.post(
								'<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>',
								{
									...
									action: 'khutar_dismiss_welcome_notice',
									success: function () {
	...
	public function change_plugin_names( $plugins ) {
		if ( array_key_exists( 'pearlfibers-companion/pearlfibers-companion.php', $plugins ) ) {
			$plugins['pearlfibers-companion/pearlfibers-companion.php']['Name'] = 'Orbit Fox Companion by khutar theme';
		}
		if ( array_key_exists( 'otter-blocks/otter-blocks.php', $plugins ) ) {
			$plugins['otter-blocks/otter-blocks.php']['Name'] = 'Gutenberg Blocks and Template Library by khutar theme';
		}
		return $plugins;
	}

	/**
	 * Import khutar options when switching to a child theme.
	 */
	public function migrate_options() {
		...
		/* import khutar options */
		$khutar_mods = get_option( 'theme_mods_khutar' );

		if ( ! empty( $khutar_mods ) ) {

			foreach ( $khutar_mods as $khutar_mod_k => $khutar_mod_v ) {
				set_theme_mod( $khutar_mod_k, $khutar_mod_v );
			}
		}
	}
}
