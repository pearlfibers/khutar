...
use khutar\Core\Styles\Frontend;
use khutar\Core\Styles\Generator;
use khutar\Core\Styles\Gutenberg;

class Dynamic_Css {
	const FRONTEND_HANDLE = 'khutar-style';
	const GUTENBERG_HANDLE = 'khutar-gutenberg-style';
	/**
	 * Generator object.
	 *
	...
	public function legacy_style() {
		$classes = apply_filters( 'khutar_filter_inline_style_classes', [], 'khutar-generated-style' );
		$mobile_css = '';
		$desktop_css = '';
		$tablet_css = '';
		...
		add_filter( 'khutar_dynamic_style_output', function ( $css ) use ( $all_css ) {
			return $all_css . $css;
		} );
	}
	...
	public function enqueue() {
		...
		if ( ! class_exists( ' khutar_Pro\Core\Generic_Style', true ) ) {
			$this->legacy_style();
		}
		...
		$_subscribers = array_merge( $_subscribers, apply_filters( 'khutar_style_subscribers', [] ) );

		$this->generator->set( $_subscribers );

		$style = apply_filters( 'khutar_dynamic_style_output', $this->generator->generate(), $is_for_gutenberg ? 'gutenberg' : 'frontend' );
	...
	public function get_fallback_font() {
		$fallback = get_theme_mod( 'khutar_fallback_font_family', 'Arial, Helvetica, sans-serif' );
