...
use khutar\Compatibility\Starter_Content;
...
class Front_End {
	...
	public function setup_theme() {
		...
		if ( ! isset( $content_width ) ) {
			$content_width = apply_filters( 'khutar_content_width', 1200 );
		}

		load_theme_textdomain( 'khutar', get_template_directory() . '/languages' );
		...
		add_filter( 'pearlfibers_gutenberg_templates', array( $this, 'add_gutenberg_templates' ) );

		$this->add_amp_support();
		$nav_menus_to_register = apply_filters(
			'khutar_register_nav_menus',
			array(
				'primary' => esc_html__( 'Primary Menu', 'khutar' ),
				'footer'  => esc_html__( 'Footer Menu', 'khutar' ),
				'top-bar' => esc_html__( 'Secondary Menu', 'khutar' ),
			)
		);
		register_nav_menus( $nav_menus_to_register );

		add_image_size( 'khutar-blog', 930, 620, true );
	...
	private function get_gutenberg_color_palette() {
		$prefix                  = ( apply_filters( 'ti_wl_theme_is_localized', false ) ? __( 'Theme', 'khutar' ) : 'khutar' ) . ' - ';
		$gutenberg_color_palette = array();
		$from_global_colors      = [
			'khutar-link-color'       => array(
				'val'   => 'var(--nv-primary-accent)',
				'label' => $prefix . __( 'Primary Accent', 'khutar' ),
			),
			'khutar-link-hover-color' => array(
				'val'   => 'var(--nv-secondary-accent)',
				'label' => $prefix . __( 'Secondary Accent', 'khutar' ),
			),
			'nv-site-bg'            => array(
				'val'   => 'var(--nv-site-bg)',
				'label' => $prefix . __( 'Site Background', 'khutar' ),
			),
			'nv-light-bg'           => array(
				'val'   => 'var(--nv-light-bg)',
				'label' => $prefix . __( 'Light Background', 'khutar' ),
			),
			'nv-dark-bg'            => array(
				'val'   => 'var(--nv-dark-bg)',
				'label' => $prefix . __( 'Dark Background', 'khutar' ),
			),
			'khutar-text-color'       => array(
				'val'   => 'var(--nv-text-color)',
				'label' => $prefix . __( 'Text Color', 'khutar' ),
			),
			'nv-text-dark-bg'       => array(
				'val'   => 'var(--nv-text-dark-bg)',
				'label' => $prefix . __( 'Text Dark Background', 'khutar' ),
			),
			'nv-c-1'                => array(
				'val'   => 'var(--nv-c-1)',
				'label' => $prefix . __( 'Extra Color 1', 'khutar' ),
			),
			'nv-c-2'                => array(
				'val'   => 'var(--nv-c-2)',
				'label' => $prefix . __( 'Extra Color 2', 'khutar' ),
			),
		];
		...
		foreach ( $from_global_colors as $slug => $args ) {
			array_push(
				...
				array(
					...
					'slug'  => esc_html( $slug ),
					'color' => khutar_sanitize_colors( $args['val'] ),
				)
			);
		}
	...
	private function add_styles() {
		if ( class_exists( 'WooCommerce', false ) ) {
			wp_register_style( 'khutar-woocommerce', khutar_ASSETS_URL . 'css/woocommerce' . ( ( khutar_DEBUG ) ? '' : '.min' ) . '.css', array(), apply_filters( 'khutar_version_filter', khutar_VERSION ) );
			wp_style_add_data( 'khutar-woocommerce', 'rtl', 'replace' );
			wp_style_add_data( 'khutar-woocommerce', 'suffix', '.min' );
			wp_enqueue_style( 'khutar-woocommerce' );
		}

		wp_register_style( 'khutar-style', get_template_directory_uri() . '/style-main' . ( ( khutar_DEBUG ) ? '' : '.min' ) . '.css', array(), apply_filters( 'khutar_version_filter', khutar_VERSION ) );
		wp_style_add_data( 'khutar-style', 'rtl', 'replace' );
		wp_style_add_data( 'khutar-style', 'suffix', '.min' );
		wp_enqueue_style( 'khutar-style' );
	}
	...
	private function add_scripts() {
		...
		wp_register_script( 'khutar-script', khutar_ASSETS_URL . 'js/build/modern/frontend.js', apply_filters( 'khutar_filter_main_script_dependencies', array() ), khutar_VERSION, true );

		wp_localize_script(
			'khutar-script',
			'khutarProperties',
			apply_filters(
				'khutar_filter_main_script_localization',
				array(
		...
		wp_enqueue_script( 'khutar-script' );
		wp_script_add_data( 'khutar-script', 'async', true );

		if ( class_exists( 'WooCommerce', false ) && is_woocommerce() ) {
			wp_register_script( 'khutar-shop-script', khutar_ASSETS_URL . 'js/build/modern/shop.js', array(), khutar_VERSION, true );
			wp_enqueue_script( 'khutar-shop-script' );
			wp_script_add_data( 'khutar-shop-script', 'async', true );
		}
	...
	public function register_sidebars() {
		$sidebars = array(
			'blog-sidebar' => esc_html__( 'Sidebar', 'khutar' ),
			'shop-sidebar' => esc_html__( 'Shop Sidebar', 'khutar' ),
		);

		$footer_sidebars = apply_filters(
			'khutar_footer_widget_areas_array',
			array(
				'footer-one-widgets'   => esc_html__( 'Footer One', 'khutar' ),
				'footer-two-widgets'   => esc_html__( 'Footer Two', 'khutar' ),
				'footer-three-widgets' => esc_html__( 'Footer Three', 'khutar' ),
				'footer-four-widgets'  => esc_html__( 'Footer Four', 'khutar' ),
			)
		);
