...
final class Manager {
	...
	public function init() {
		 ...
		 * Gtb meta
		 */
		add_action( 'init', array( $this, 'khutar_register_meta' ) );
	...
	public function define_controls() {
		$this->control_classes = array(
			'khutar\\Admin\\Metabox\\Main',
		);

		$this->control_classes = apply_filters( 'khutar_filter_metabox_controls', $this->control_classes );
	}
	...
	public function add() {
		...
		add_meta_box(
			'khutar-page-settings',
			sprintf(
			/* translators: %s - post type */
				__( '%s Settings', 'khutar' ),
				$post_type
			),
	...
	public function render_metabox_notice() {
		...
		echo '<h4>' . esc_html__( 'Page Settings are now accessible from the top bar', 'khutar' ) . '</h4>';
		printf(
			/* translators: %1$s - Keyboard shortcut.   %2&s - svg icon */
			esc_html__( 'Click the %1$s icon in the top bar or use the keyboard shortcut ( %2$s ) to customise the layout settings for this page', 'khutar' ),
			...
			'<strong>SHIFT + ALT + S</strong> ' . esc_html__( 'or', 'khutar' ) . ' <strong>control + option + S</strong>'
		);
		echo '</div>';
	}
	...
	public function enqueue() {
		...
		wp_register_script( 'khutar-metabox', khutar_ASSETS_URL . 'js/build/all/metabox.js', array( 'jquery' ), khutar_VERSION, true );

		wp_localize_script( 'khutar-metabox', 'khutarMetabox', $this->get_localization() );

		wp_enqueue_script( 'khutar-metabox' );
	 ...
	 * Register meta
	 */
	public function khutar_register_meta() {
		$meta_sidebar_controls = apply_filters(
			'khutar_sidebar_meta_controls',
			[
				[
	...
	public function meta_sidebar_script_enqueue() {
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'khutar-meta-sidebar', 'khutar' );
		}
		...
		$localized_data = apply_filters(
			'khutar_meta_sidebar_localize_filter',
			array(
				'actions'              => array(
					'khutar_meta_content_width' => array(
						'container' => $container,
						'editor'    => $editor_width,
		...
		wp_enqueue_style(
			'khutar-meta-sidebar-css', // Handle.
	...
	private function get_post_elements_default_order() {
		...
		$content_order = get_theme_mod( 'khutar_layout_single_post_elements_order', wp_json_encode( $default_order ) );
	...
	public function set_page_width( $post_id, $post ) {
		...
		$checkout_was_updated = get_post_meta( $post_id, 'khutar_checkout_updated', 'no' );
		if ( Main::is_new_page() || ( Main::is_checkout() && $checkout_was_updated === 'no' ) ) {
			update_post_meta( $post_id, 'khutar_meta_sidebar', 'full-width' );
			update_post_meta( $post_id, 'khutar_meta_enable_content_width', 'on' );
			update_post_meta( $post_id, 'khutar_meta_content_width', 100 );
			if ( Main::is_checkout() ) {
				update_post_meta( $post_id, 'khutar_checkout_updated', 'yes' );
			}
		}
	}
}
