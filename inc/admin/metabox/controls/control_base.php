...
abstract class Control_Base {
	...
	public function render( $post_id ) {
		...
		wp_nonce_field( 'khutar_meta_box_nonce', 'khutar_meta_box_process' );
	}
	...
	final public function save( $post_id ) {
		if ( ! isset( $_POST['khutar_meta_box_process'] ) ) {
			return;
		}
		if ( ! wp_verify_nonce( sanitize_key( $_POST['khutar_meta_box_process'] ), 'khutar_meta_box_nonce' ) ) {
			return;
		}
		...
		if ( isset( $_POST[ $this->id ] ) ) {
			...
			if ( $this->id === 'khutar_meta_enable_content_width' ) {
				update_post_meta( $post_id, 'khutar_meta_enable_content_width', 'off' );
			} else {
				delete_post_meta( $post_id, $this->id );
			}
			return;
