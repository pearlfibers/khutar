/* global khutarProperties,_wpCustomizeSettings,parent,Masonry,imagesLoaded */
 ...
 */
const masonry = () => {
	const { masonryStatus, masonryColumns, blogLayout } = khutarProperties;
...
const infiniteScroll = () => {
	if (khutarProperties.infScroll !== 'enabled') {
		return;
	}
...
const requestMorePosts = () => {
	...
	if (page > khutarProperties.maxPages) {
		trigger.parentNode.removeChild(trigger);
		document.querySelector('.nv-loader').style.display = 'none';
		return;
	}
	const blog = document.querySelector(postWrapSelector);
	const lang = khutarProperties.lang;
	const baseUrl = khutarProperties.endpoint + page;
	...
	httpGetAsync(
		...
		(response) => {
			...
			if (khutarProperties.masonryStatus !== 'enabled') {
				return false;
			}
			window.nvMasonry.reloadItems();
			window.nvMasonry.layout();
		},
		khutarProperties.query
	);
};
