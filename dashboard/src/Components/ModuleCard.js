...
const ModuleCard = ({
	...
	const {
		...
		required_actions,
	} = khutarDash.modules[slug];
	const { upgradeLinks } = khutarDash;
...
export default compose(
	withSelect((select) => {
		const { getModuleStatus, getLicenseTier } = select('khutar-dashboard');
		return {
	...
	withDispatch((dispatch) => {
		const { changeModuleStatus, setToast } = dispatch('khutar-dashboard');
		return {
