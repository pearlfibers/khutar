...
const requestData = async (
	...
	if (useNonce) {
		options.headers['x-wp-nonce'] = khutarDash.nonce;
	}
...
const isValid = (option, optionsArray) => {
	if ('nv_pro_typekit_id' === option) {
		if (!optionsArray.khutar_pro_typekit_data) {
			return {
				message: __('Typekit Project ID is invalid', 'khutar'),
				success: false,
			};
		}
	}
