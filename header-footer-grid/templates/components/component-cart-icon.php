...
if ( $cart_style === 'off-canvas' ) {
	...
	$off_canvas_closing_button = '<div class="cart-off-canvas-button-wrapper"><span class="nv-close-cart-sidebar button button-secondary secondary-default">' . __( 'Close', 'khutar' ) . '</span></div>';
}
...
<div class="component-wrap">
	...
	">
		<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="cart-icon-wrapper">
			...
			?>
			<?php khutar_cart_icon( true, 15, $icon_type ); ?>
			<span class="screen-reader-text">
				<?php esc_html_e( 'Cart', 'khutar' ); ?>
			</span>
			<span class="cart-count">
				<?php echo esc_html( WC()->cart->get_cart_contents_count() ); ?>
			</span>
			<?php do_action( 'khutar_cart_icon_after_cart_total' ); ?>
		</a>
		...
		<div class="<?php echo esc_attr( implode( ' ', $mini_cart_classes ) ); ?>">
			...
			if ( ! empty( $custom_html ) ) {
				...
				echo wp_kses( balanceTags( apply_filters( 'khutar_post_content', $custom_html ), true ), $allowed_post_tags );
				echo '</div>';
			}
