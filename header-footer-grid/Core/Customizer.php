...
class Customizer {
	...
	public function __construct() {
		...
		add_filter( 'khutar_react_controls_localization', array( $this, 'add_dynamic_tags_options' ) );
	}
					...
					<div class="hfg--cb-header">
						...
						<div class="hfg--cb-notice conditional-header hidden">
							...
							<?php
								...
								echo wp_kses_post( sprintf( __( 'You are customizing the %s Header', 'khutar' ), ' <a>' . __( 'Default', 'khutar' ) . '</a> ' ) );
							?>
								</p>
						</div>
						<# } #>
						<div class="hfg--cb-notice welcome-notice {{data.id}} hidden">
							<p>
								...
								<span><?php echo sprintf( esc_html__( '%s Builder:', 'khutar' ), '{{data.title}}' ); ?></span>
							<?php
								/* translators: %s is the header name */
								echo esc_html__( 'Click on any empty space to add components, or existing components to adjust settings.', 'khutar' );
							?>
						...
						<div class="hfg--cb-actions">
							...
							<a class="button button-secondary hfg--panel-close" href="#">
								<span class="close-text"><i class="dashicons dashicons-arrow-down-alt2"
										style="margin-top: 4px;"></i> <?php esc_html_e( 'Close', 'khutar' ); ?></span>
				...
				<div class="hfg-widgets-panel-header">
					<div class="hfg-component-search">
						...
						<input class="component-search" type="search"
							placeholder="<?php esc_attr_e( 'Search Components', 'khutar' ); ?>..."/>
					</div>
