...
class Footer extends Abstract_Builder {
	...
	public function init() {
		$this->set_property( 'title', __( 'Footer', 'khutar' ) );
		$this->set_property(
			'description',
			apply_filters(
		...
		$this->set_property(
			...
			array(
				...
				/* translators: 1: builder, 2: builder symbol */
					esc_attr__( 'Welcome to the %1$s builder! Click the %2$s button to add a new component or follow the Quick Links.', 'khutar' ),
					$this->get_property( 'title' ),
					'+'
				),
				'quickLinks'  => array(
					'footer_copyright_content'            => array(
						'label' => esc_html__( 'Change Copyright', 'khutar' ),
						'icon'  => 'dashicons-nametag',
					),
					'hfg_footer_layout_bottom_background' => array(
						'label' => esc_html__( 'Change Footer Color', 'khutar' ),
	...
	protected function get_rows() {
		...
		return [
			'top'    => array(
				'title'       => __( 'Footer Top', 'khutar' ),
				'description' => $this->get_property( 'description' ),
			),
			'bottom' => array(
				'title'       => __( 'Footer Bottom', 'khutar' ),
				'description' => $this->get_property( 'description' ),
			),
		];
	}
