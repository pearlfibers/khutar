...
use khutar\Customizer\Controls\Tabs;
		...
		$args = array(
			...
			'tabs'     => array(
				self::TAB_GENERAL => array(
					'label' => esc_html__( 'General', 'khutar' ),
					'icon'  => 'admin-generic',
				),
				self::TAB_LAYOUT  => array(
					'label' => esc_html__( 'Layout', 'khutar' ),
					'icon'  => 'layout',
				),
				self::TAB_STYLE   => array(
					'label' => esc_html__( 'Style', 'khutar' ),
	...
	public function add( $arguments = array() ) {
		...
		if ( isset( $arguments['conditional_header'] ) && $arguments['conditional_header'] === true ) {
			...
			if ( defined( 'khutar_PRO_VERSION' ) ) {
				add_filter(
					'khutar_pro_react_controls_localization',
					function ( $array ) use ( $id ) {
						$array['headerControls'][] = $id;
