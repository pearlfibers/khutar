...
class Copyright extends Abstract_Component {
	...
	public function init() {
		$this->set_property( 'label', __( 'Copyright', 'khutar' ) );
			...
			[
				...
				'default'            => get_theme_mod(
					...
					apply_filters(
						'ti_wl_copyright',
						sprintf(
						/* translators: %1$s is Theme Name ( khutar ), %2$s is WordPress */
							esc_html__( '%1$s | Powered by %2$s', 'khutar' ),
							wp_kses_post( '<p><a href="https://pearlfibers.com/themes/khutar/" rel="nofollow">khutar</a>' ),
							wp_kses_post( '<a href="http://wordpress.org" rel="nofollow">WordPress</a></p>' )
						)
					)
				),
				'label'              => __( 'Copyright', 'khutar' ),
				'type'               => 'textarea',
