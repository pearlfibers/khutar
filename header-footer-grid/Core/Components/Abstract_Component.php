...
abstract class Abstract_Component implements Component {
	...
	public function define_settings() {
		...
		$align_choices   = [
			'left'   => [
				'tooltip' => __( 'Left', 'khutar' ),
				'icon'    => 'editor-alignleft',
			],
			'center' => [
				'tooltip' => __( 'Center', 'khutar' ),
				'icon'    => 'editor-aligncenter',
			],
			'right'  => [
				'tooltip' => __( 'Right', 'khutar' ),
				'icon'    => 'editor-alignright',
			],
		];
		if ( strpos( $this->get_id(), Button::COMPONENT_ID ) > -1 ) {
			$align_choices['justify'] = [
				'tooltip' => __( 'Justify', 'khutar' ),
				'icon'    => 'editor-justify',
			];
		}
		...
		if ( $this->get_id() !== Search::COMPONENT_ID ) {
			SettingsManager::get_instance()->add(
				[
					...
					'label'                 => __( 'Alignment', 'khutar' ),
					'type'                  => '\khutar\Customizer\Controls\React\Responsive_Radio_Buttons',
	...
	private function add_typography_controls() {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'transport'         => 'postMessage',
				'priority'          => $priority,
				'type'              => 'khutar\Customizer\Controls\Heading',
				'sanitize_callback' => 'sanitize_text_field',
				'label'             => __( 'Typography', 'khutar' ),
				'section'           => $this->section,
				'options'           => [
					'accordion'        => true,
		...
		if ( $this->has_typeface_control ) {
			SettingsManager::get_instance()->add(
				[
					...
					'transport'             => 'postMessage',
					'priority'              => $priority + 2,
					'type'                  => '\khutar\Customizer\Controls\React\Typography',
					'live_refresh_selector' => $this->default_typography_selector,
					'section'               => $this->section,
					'default'               => $this->typography_default,
					'sanitize_callback'     => 'khutar_sanitize_typography_control',
					'options'               => [
						'input_attrs' => array(
							'size_units'             => [ 'em', 'px' ],
	...
	private function add_vertical_alignment_control() {
		...
		$align_choices = [
			'top'    => [
				'tooltip' => __( 'Top', 'khutar' ),
				'icon'    => 'arrow-up',
			],
			'middle' => [
				'tooltip' => __( 'Middle', 'khutar' ),
				'icon'    => 'sort',
			],
			'bottom' => [
				'tooltip' => __( 'Bottom', 'khutar' ),
				'icon'    => 'arrow-down',
			],
		];
		...
		SettingsManager::get_instance()->add(
			[
				...
				'default'               => 'middle',
				'label'                 => __( 'Vertical Alignment', 'khutar' ),
