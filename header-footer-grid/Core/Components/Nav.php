...
class Nav extends Abstract_Component {
	...
	const LAST_ITEM_ID     = 'khutar_last_menu_item';
	...
	public function init() {
		$this->set_property( 'label', __( 'Primary Menu', 'khutar' ) );
		...
		add_filter(
			'khutar_last_menu_setting_slug_' . $this->get_class_const( 'COMPONENT_ID' ),
			array(
				$this,
				'filter_khutar_last_menu_setting_slug',
			)
		);
	}
	 ...
	 * @access  public
	 */
	public function filter_khutar_last_menu_setting_slug( $slug ) {
	...
	public function add_settings() {
		SettingsManager::get_instance()->add(
			[
				...
				'label'              => __( 'Hover Skin Mode', 'khutar' ),
				'type'               => '\khutar\Customizer\Controls\React\Radio_Buttons',
		...
		SettingsManager::get_instance()->add(
			[
				...
				'live_refresh_css_prop' => [
					'template' =>
						$selector . ' li:not(.current_page_item):not(.current-menu-item):not(.woocommerce-mini-cart-item) > a,' . $selector . ' li.khutar-mm-heading span {
						color: {{value}};
					}',
				],
			]
		...
		SettingsManager::get_instance()->add(
			[
				...
				'label'                 => __( 'Active Item Color', 'khutar' ),
				'type'                  => 'khutar_color_control',
				'section'               => $this->section,
				'conditional_header'    => true,
				'live_refresh_selector' => true,
		...
		SettingsManager::get_instance()->add(
			[
				...
				'label'                 => __( 'Items Hover Color', 'khutar' ),
				'type'                  => 'khutar_color_control',
				'section'               => $this->section,
				'conditional_header'    => true,
				'live_refresh_selector' => true,
		...
		$components               = array(
			'search' => __( 'Search', 'khutar' ),
		);

		if ( class_exists( 'WooCommerce', false ) ) {
			array_push( $order_default_components, 'cart' );
			$components['cart'] = __( 'Cart', 'khutar' );
		}

		$components = apply_filters( 'khutar_last_menu_item_components', $components );
		 ...
		 * @since 2.5.3
		 */
		$old_last_menu_item = json_decode( get_theme_mod( 'khutar_last_menu_item' ) );
		if ( $old_last_menu_item !== false && ! empty( $old_last_menu_item ) ) {
			SettingsManager::get_instance()->add(
				[
					...
					'label'             => __( 'Last Menu Item', 'khutar' ),
					'type'              => 'khutar\Customizer\Controls\Ordering',
					'options'           => [
		...
		SettingsManager::get_instance()->add(
			[
				...
				'type'              => '\khutar\Customizer\Controls\Button',
				'options'           => [
					'button_text'  => __( 'Primary Menu', 'khutar' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'section'            => $this->section,
				'label'              => __( 'Items Spacing (px)', 'khutar' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'group'              => $this->get_class_const( 'COMPONENT_ID' ),
				'tab'                => SettingsManager::TAB_LAYOUT,
				'label'              => __( 'Items Min Height (px)', 'khutar' ),
				'sanitize_callback'  => [ $this, 'sanitize_responsive_int_json' ],
				'transport'          => 'post' . $this->get_class_const( 'COMPONENT_ID' ),
		...
		SettingsManager::get_instance()->add(
			[
				...
				'default'            => 0,
				'label'              => __( 'Expand first level of dropdowns when menu is in mobile menu content.', 'khutar' ),
	...
	public function add_style( array $css_array = array() ) {
		...
		$css_array[] = [
			Dynamic_Selector::KEY_SELECTOR => $selector . 'li:not(.woocommerce-mini-cart-item) > a,' . $selector . '.has-caret > a,' . $selector . ' .khutar-mm-heading span,' . $selector . ' .has-caret',
		...
		$css_array[] = [
			Dynamic_Selector::KEY_SELECTOR => '.builder-item--' . $this->get_id() . ' .nav-ul li a, .builder-item--' . $this->get_id() . ' .khutar-mm-heading span',
			Dynamic_Selector::KEY_RULES    => [
				'min-height' => [
