...
class CartIcon extends Abstract_Component {
	...
	public function init() {
		$this->set_property( 'label', __( 'Cart Icon', 'khutar' ) );
		...
		if ( function_exists( 'do_blocks' ) ) {
			add_filter( 'khutar_post_content', 'do_blocks' );
		}
		add_filter( 'khutar_post_content', 'wptexturize' );
		add_filter( 'khutar_post_content', 'convert_smilies' );
		add_filter( 'khutar_post_content', 'convert_chars' );
		add_filter( 'khutar_post_content', 'wpautop' );
		add_filter( 'khutar_post_content', 'shortcode_unautop' );
		add_filter( 'khutar_post_content', 'do_shortcode' );
	}
	...
	public function add_settings() {
		...
		SettingsManager::get_instance()->add(
			[
				...
				'default'               => 15,
				'label'                 => __( 'Icon Size', 'khutar' ),
				'type'                  => 'khutar\Customizer\Controls\React\Range',
				'options'               => [
					'input_attrs' => [
						'min'        => 10,
						'max'        => 100,
	...
	public function add_style( array $css_array = array() ) {
		$css_array[] = [
			...
			Dynamic_Selector::KEY_RULES    => [
				\khutar\Core\Settings\Config::CSS_PROP_WIDTH => [
				...
				\khutar\Core\Settings\Config::CSS_PROP_HEIGHT => [
					Dynamic_Selector::META_KEY     => $this->get_id() . '_' . self::SIZE_ID,
					Dynamic_Selector::META_DEFAULT => SettingsManager::get_instance()->get_default( $this->get_id() . '_' . self::SIZE_ID ),
				],
				\khutar\Core\Settings\Config::CSS_PROP_FILL_COLOR => [
		...
		$css_array[] = [
			Dynamic_Selector::KEY_SELECTOR => $this->default_selector . ' .cart-icon-label',
			Dynamic_Selector::KEY_RULES    => [
				\khutar\Core\Settings\Config::CSS_PROP_FONT_SIZE => [
	...
	public static function should_load_pro_features() {
		if ( ! class_exists( '\khutar_Pro\Modules\Woocommerce_Booster\Customizer\Cart_Icon' ) ) {
			return false;
		}
